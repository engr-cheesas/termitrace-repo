    ...
    <script>
        $(document).ready(function() {
            $("#sendCommand").click(function() {
                ...
                var eventSource = new EventSource("serial.php");  // Open event stream

                eventSource.onmessage = function(event) {
                    $("#output").append(event.data + "<br>");  // Append each response
                    if (event.data === "DONE") {
                        eventSource.close();  // Stop reading after completion
                    }
                };

                eventSource.onerror = function() {
                    $("#output").append("⚠ ERROR: Failed to communicate with Arduino.<br>");
                    eventSource.close();
                };
            });
        });
